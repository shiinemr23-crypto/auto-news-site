...
export function LoadMore({ initial, before }: { initial: Article[]; before?: string }) {
  const [stories, setStories] = useState(initial); const [loading, setLoading] = useState(false); const [done, setDone] = useState(false);
  async function more() { const last = stories.at(-1)?.createdAt ?? before; if (!last) return setDone(true); setLoading(true); try { const result = await getDocs(query(collection(db, "articles"), where("created_at", "<", Timestamp.fromDate(new Date(last))), orderBy("created_at", "desc"), limit(9))); const next = result.docs.map((item) => asArticle(item.id, item.data())); setStories((current) => [...current, ...next]); setDone(next.length < 9); } finally { setLoading(false); } }
