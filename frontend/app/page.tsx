...
export default async function HomePage() {
  ...
  return <div className="page-wrap home-page">
    ...
    <section className="feed-layout"><div className="latest-column"><section className="section-head latest-head"><p className="eyebrow">Latest dispatches</p><h2>The newsfeed</h2></section>{today.length > 0 && <NewsGroup label="Today" stories={today} />}{thisWeek.length > 0 && <NewsGroup label="This week" stories={thisWeek} />}{latest.length === 0 && <p className="quiet-note">New stories will appear here as they arrive.</p>}</div><aside className="trending-sidebar"><p className="eyebrow">Trending now</p><h2>On the radar</h2>{trending.map((story, index) => <Link className="trending-story" href={`/article/${story.id}`} key={story.id}><span>0{index + 1}</span><div><p>{story.source}</p><h3>{story.title}</h3></div></Link>)}</aside></section>
  </div>;
}
